...
def test_wearable_gps(record=False):
    if record:
        clock = Clock()
        sensor_set = WearableSensorSet()
        location = []
        while True:
            data = sensor_set.get()
            if 0 in data.keys():
                print(data[0].gps_position[:2])
                location.append(data[0].gps_position[:2])
                torch.save(location, 'location.pt')
                clock.tick(1)
    else:
        import folium
        gps_coordinates = torch.load('location.pt')
        mymap = folium.Map(location=gps_coordinates[0], zoom_start=15)
        for coord in gps_coordinates[::5]:
            folium.Marker(coord).add_to(mymap)
        folium.PolyLine(gps_coordinates, color='blue', weight=2.5, opacity=1).add_to(mymap)
        mymap.save('outdoor_walk_map.html')
